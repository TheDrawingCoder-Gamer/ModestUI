...
object Rect {
  ...
  sealed case class BuildOps[F[_]](val underlying: Boolean = true) extends AnyVal {
    def apply[T](paint: Paint, child: T)(using F: Async[F], C: Component[F, T]): Resource[F, Rect[F, T]] =
      Ref[F].of(IRect(0, 0, 0, 0)).flatMap { childRect =>
        F.delay { new Rect(paint, child, childRect) }
      }.toResource
    def apply[T](child: T)(using F: Async[F], C: Component[F, T]): Resource[F, Rect[F, T]] =
      apply(new Paint(), child)
  }
  def apply[F[_]] = new BuildOps[F]
}

given rect_Component[F[_], T](using F: Async[F], C: Component[F, T]): Component[F, Rect[F, T]] with
...
object RoundedRect {
  // TRUE!
  sealed case class BuildOps[F[_]](val underlying: Boolean = true) extends AnyVal {
    def apply[T](radius: Float, paint: Paint, child: T)(using F: Async[F], C: Component[F, T]): Resource[F, RoundedRect[F, T]] =
      Ref[F].of(IRect(0, 0, 0, 0)).flatMap { childRect =>
        F.delay { new RoundedRect(radius, paint, child, childRect) }
      }.toResource
    def apply[T](radius: Float, child: T)(using F: Async[F], C: Component[F, T]): Resource[F, RoundedRect[F, T]] =
      apply(radius, new Paint(), child)
  }
  def apply[F[_]] = new BuildOps[F]
}

given roundedrect_Component[F[_], T](using F: Async[F], C: Component[F, T]): Component[F, RoundedRect[F, T]] with
